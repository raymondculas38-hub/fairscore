...
@section('title', 'Scoring — ' . $event->name)
@section('container-class', 'w-full max-w-7xl mx-auto')
...
<div class="mb-4 flex items-center justify-between gap-4 flex-wrap">
    <div class="flex items-center gap-3">
        <a href="{{ route('judge.dashboard') }}" class="btn-secondary text-xs py-1.5 px-3">← Events</a>
        <h2 class="font-black text-xl truncate" style="color:#f1f5f9;">{{ $event->name }}</h2>
    ...
    @php
        ...
        $filledScores = $existingScores->count();
    ...
    <div class="w-full sm:w-64 glass-card px-3 py-2 flex-shrink-0">
        <div class="flex items-center justify-between mb-1.5">
            ...
            <span class="text-xs font-bold" id="progress-text" style="color:#f59e0b;">{{ $filledScores }}/{{ $totalScores }} ({{ $progress }}%)</span>
        </div>
        <div class="w-full h-1.5 rounded-full" style="background:rgba(255,255,255,0.08);">
            <div id="progress-bar" class="h-1.5 rounded-full transition-all duration-500"
                 style="width:{{ $progress }}%;background:linear-gradient(90deg,#f59e0b,#2dd4bf);"></div>
        </div>
    </div>
</div>
                ...
                <tr>
                    ...
                    @foreach($criteria as $c)
                        <th class="text-right whitespace-nowrap" style="min-w: 110px;">
                            <div class="font-bold" style="color:#cbd5e1;">{{ $c->name }}</div>
                            <div class="text-[0.65rem]" style="color:#64748b;">Max: {{ number_format($c->max_score, 0) }}</div>
                        </th>
                    @endforeach
            ...
            <tbody class="divide-y" style="divide-color:rgba(255,255,255,0.04);">
                @forelse($participants as $participant)
                    <tr class="hover:bg-white/5 transition-colors group">
                        ...
                        @foreach($criteria as $c)
                            ...
                            <td class="text-right">
                                <div class="relative inline-block w-24 group-hover:-translate-y-0.5 transition-transform">
                                    <input type="number" 
                                           class="form-input score-input text-right font-bold w-full {{ $existing ? 'ring-1 ring-green-500/30' : '' }}" 
                        ...
                        {{-- Auto Computed Total --}}
                        <td class="text-right">
                            <span id="total-{{ $participant->id }}" class="text-lg font-black" style="color:#f59e0b;">
                ...
                @empty
                    <tr>
                        <td colspan="{{ count($criteria) + 3 }}" class="text-center py-12" style="color:#64748b;">
...
const totalExpectedScores = {{ $totalScores }};
let currentFilledScores = {{ $filledScores }};
const debounceMap = {};

// Cache existing filled states to track progress changes accurately
const filledKeys = new Set(
    @json($existingScores->keys())
);
