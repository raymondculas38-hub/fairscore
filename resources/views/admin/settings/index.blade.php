...
@section('page-subtitle', 'Configure global event parameters and branding')
...
<div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
    ...
    <div class="lg:col-span-2 space-y-6">
        <form action="{{ route('admin.settings.update') }}" method="POST" class="bg-[#0b1426] border border-[#1e293b] rounded-xl overflow-hidden">
            @csrf
            ...
            <div class="p-6 border-b border-[#1e293b]">
                ...
                <div class="mt-6 space-y-4">
                    <div>
                        ...
                        <input type="text" name="settings[event_title]" value="{{ $settings['event_title']->value ?? 'FairScore Event' }}"
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            ...
                            <div class="flex items-center gap-3">
                                <input type="color" name="settings[theme_primary_color]" value="{{ $settings['theme_primary_color']->value ?? '#f59e0b' }}"
                                       class="h-10 w-14 rounded cursor-pointer bg-transparent border-0 p-0">
                                <span class="text-xs text-slate-400 font-mono uppercase">{{ $settings['theme_primary_color']->value ?? '#f59e0b' }}</span>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="mt-6 space-y-4">
                    <label class="flex items-center gap-3 p-3 rounded-lg border border-[#1e293b] bg-[#050c1a] cursor-pointer hover:border-slate-700 transition">
                        ...
                        <input type="checkbox" name="settings[allow_score_editing]" value="1" 
                            {{ ($settings['allow_score_editing']->value ?? '1') == '1' ? 'checked' : '' }}
                    ...
                    <label class="flex items-center gap-3 p-3 rounded-lg border border-[#1e293b] bg-[#050c1a] cursor-pointer hover:border-slate-700 transition">
                        ...
                        <input type="checkbox" name="settings[require_judge_comments]" value="1" 
                            {{ ($settings['require_judge_comments']->value ?? '0') == '1' ? 'checked' : '' }}
                            class="w-5 h-5 rounded border-slate-700 bg-slate-800 text-amber-500 focus:ring-amber-500 focus:ring-offset-slate-900">
                        <div>
        ...
        <div class="bg-[#1a0f14] border border-red-900/50 rounded-xl overflow-hidden">
            <div class="p-6 border-b border-red-900/50">
                ...
                <div class="mt-6 pt-4 border-t border-red-900/50">
                    ...
                    <form action="{{ route('admin.settings.factory_reset') }}" method="POST" onsubmit="return confirm('Are you absolutely sure? This will delete ALL scores permanently.');">
                        @csrf
