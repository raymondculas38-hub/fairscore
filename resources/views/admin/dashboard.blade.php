...
<div class="grid grid-cols-2 gap-4 mb-6 lg:grid-cols-4">
    ...
    @foreach($cards as $i => $card)
        <div class="stat-card animate-fade-in-up delay-{{ $i+1 }}">
            <div class="flex items-center justify-between mb-3">
                <span class="text-xs font-semibold uppercase tracking-widest" style="color:#475569;">{{ $card['label'] }}</span>
                <div class="w-9 h-9 rounded-xl flex items-center justify-center" style="background:{{ $card['bg'] }};">
                    <svg class="w-4.5 h-4.5" fill="none" viewBox="0 0 24 24" stroke="{{ $card['color'] }}" stroke-width="1.8"><path stroke-linecap="round" stroke-linejoin="round" d="{{ $card['icon'] }}" /></svg>
                </div>
            </div>
            <div class="text-3xl font-black" style="color:{{ $card['color'] }};">{{ $card['value'] }}</div>
        </div>
    @endforeach
</div>
...
<div class="grid grid-cols-1 gap-5 lg:grid-cols-5">
    ...
    <div class="glass-card lg:col-span-3 animate-fade-in-up delay-3">
        <div class="flex items-center justify-between px-5 py-4 border-b" style="border-color:rgba(255,255,255,0.06);">
            ...
            <a href="{{ route('admin.events.index') }}" class="text-xs font-semibold" style="color:#f59e0b;">View all →</a>
        </div>
        <div class="divide-y" style="divide-color:rgba(255,255,255,0.04);">
            @forelse($activeEvents as $event)
                <div class="flex items-center gap-4 px-5 py-3.5">
                    <div class="flex-1 min-w-0">
                        <p class="text-sm font-semibold truncate" style="color:#e2e8f0;">{{ $event->name }}</p>
                    ...
                    <a href="{{ route('admin.events.edit', $event) }}" class="btn-teal">Manage</a>
                </div>
            @empty
                <div class="px-5 py-10 text-center" style="color:#334155;">
                    ...
                    <a href="{{ route('admin.events.create') }}" class="btn-primary inline-flex mt-3">Create your first event</a>
                </div>
            @endforelse
        </div>
    </div>

    {{-- Live Events Quick Links --}}
            ...
            @forelse($liveEvents as $event)
                <div class="p-3 rounded-xl" style="background:rgba(34,197,94,0.06);border:1px solid rgba(34,197,94,0.15);">
                    <div class="flex items-center justify-between mb-2">
                        <p class="text-sm font-semibold" style="color:#e2e8f0;">{{ $event->name }}</p>
                        <span class="badge-live">LIVE</span>
                    </div>
...
@php $firstLiveEvent = $liveEvents->first(); @endphp
<div class="mt-6 animate-fade-in-up delay-5">
    <div class="flex items-center justify-between mb-4">
        <h2 class="text-lg font-bold" style="color:#f1f5f9;">Judge Status: {{ $firstLiveEvent->name }}</h2>
        <a href="{{ route('leaderboard.show', $firstLiveEvent) }}" target="_blank" class="text-sm font-semibold hover:underline" style="color:#2dd4bf;">Open Scoreboard →</a>
    </div>
        ...
        <div class="overflow-x-auto">
            <table class="w-full text-left text-sm" style="color:#cbd5e1;">
                ...
                <tbody class="divide-y" style="divide-color:rgba(255,255,255,0.04);">
                    @forelse($firstLiveEvent->judges as $judge)
                        @php
                            $hasSubmitted = $firstLiveEvent->scores->where('judge_id', $judge->id)->isNotEmpty();
                        @endphp
                        <tr class="hover:bg-white/5 transition-colors">
                            <td class="px-5 py-3 flex items-center gap-3">
                                ...
                                <span class="font-medium text-slate-200">{{ $judge->name }}</span>
                            </td>
                            <td class="px-5 py-3 text-center">
                                @if($hasSubmitted)
                                    <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-[10px] font-bold uppercase tracking-widest bg-green-500/20 text-green-400 border border-green-500/30">
                                        Submitted
                                    </span>
                                @else
                            ...
                            <td class="px-5 py-3 text-right">
                                @if(!$hasSubmitted)
                                    <form action="{{ route('admin.events.remind', [$firstLiveEvent->id, $judge->id]) }}" method="POST" class="inline">
                                        @csrf
