                ...
                <div class="group relative bg-[#0b1426]/80 backdrop-blur-md rounded-2xl border border-white/5 p-6 hover:border-amber-500/50 hover:shadow-lg hover:shadow-amber-500/10 transition-all duration-300">
                    ...
                    <div class="flex justify-between items-start mb-4">
                        <div class="w-12 h-12 rounded-full bg-slate-800 flex items-center justify-center text-xl shadow-inner border border-white/5">
                            <?= htmlspecialchars((string)($event->status === 'live' ? '🔴' : '📅')) ?>
                        </div>
                    ...
                    <h3 class="text-xl font-bold text-slate-100 mb-2 truncate"><?= htmlspecialchars((string)($event->name)) ?></h3>
                    <p class="text-sm text-slate-500 line-clamp-2 h-10 mb-4"><?= htmlspecialchars((string)($event->description ?? 'No description provided.')) ?></p>
                    ...
                    <div class="flex items-center gap-4 text-xs font-medium text-slate-400 mb-6">
                        <div class="flex items-center gap-1.5">
                            ...
                            <?= htmlspecialchars((string)($event->participants_count)) ?> Contestants
                        </div>
                    </div>

                    <?php if($event->status === 'live'): ?>
                        <div class="flex flex-col gap-2">
                            <a href="<?= htmlspecialchars((string)(route('judge.score', $event))) ?>" class="w-full inline-flex justify-center items-center gap-2 bg-amber-600 hover:bg-amber-500 text-white font-bold px-4 py-2.5 rounded-xl transition-all shadow-md">
                                Evaluate
                                <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M14 5l7 7m0 0l-7 7m7-7H3" /></svg>
                            </a>
                            <form action="<?= htmlspecialchars((string)(route('judge.event.leave', $event))) ?>" method="POST" class="m-0 w-full" onsubmit="return confirm('Are you sure you want to leave this event? You will no longer be able to judge it.');">
                                <?= csrf_field() ?>
                                <button type="submit" class="w-full inline-flex justify-center items-center gap-2 bg-slate-800 hover:bg-red-500/20 text-slate-400 hover:text-red-400 font-bold px-4 py-2 rounded-xl transition-all border border-transparent hover:border-red-500/30 text-xs">
                    ...
                    <?php else: ?>
                        <div class="flex flex-col gap-2">
                            <button disabled class="w-full inline-flex justify-center items-center gap-2 bg-slate-800 text-slate-500 font-bold px-4 py-2.5 rounded-xl transition-all cursor-not-allowed">
                                <?= htmlspecialchars((string)($event->status === 'completed' ? 'Scoring Closed' : 'Waiting for Admin...')) ?>
                            </button>
                            <form action="<?= htmlspecialchars((string)(route('judge.event.leave', $event))) ?>" method="POST" class="m-0 w-full" onsubmit="return confirm('Are you sure you want to leave this event? You will no longer be able to judge it.');">
                                <?= csrf_field() ?>
                                <button type="submit" class="w-full inline-flex justify-center items-center gap-2 bg-slate-800 hover:bg-red-500/20 text-slate-400 hover:text-red-400 font-bold px-4 py-2 rounded-xl transition-all border border-transparent hover:border-red-500/30 text-xs">
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        if (globalStartBtn) {
            globalStartBtn.addEventListener('click', (e) => {
                ...
                if (window.location.pathname === '<?= htmlspecialchars((string)(route('judge.dashboard', [], false))) ?>' 
